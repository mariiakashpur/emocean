...
df = pd.read_csv(data_file).drop(['ID'], axis=1)
...
def get_bow(X_train, X_test):
    ...
    X_train_sparse = sp.sparse.hstack((tfidf_vect.fit_transform(X_train['text']), X_train.drop(['text'], axis=1).values), format='csr')
    X_test_sparse = sp.sparse.hstack((tfidf_vect.transform(X_test['text']), X_test.drop(['text'], axis=1).values), format='csr')
...
X_train_sparse, X_test_sparse = get_bow(X_train, X_test)
model = ModelDD(X_train_sparse, y_train, X_test_sparse, y_test)
